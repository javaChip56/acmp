...
public sealed class ClientPackageCacheOptions
{
...
public sealed class EncryptedFileClientCredentialStore : IDisposable
{
    private readonly ClientPackageCacheOptions _options;
    ...
    public EncryptedFileClientCredentialStore(
        ClientPackageCacheOptions options,
    ...
    public async Task<ClientSigningCredentialPackage> GetByKeyIdAsync(
        string keyId,
        string? expectedKeyVersion = null,
        CancellationToken cancellationToken = default)
    {
        var normalizedKeyId = RequireKeyId(keyId);
        var packagePath = Path.Combine(RequireDirectory(), $"{normalizedKeyId}.client.acmppkg.json");
        EnsureWatcher();
        ...
        try
        {
            var package = await _packageReader.ReadClientSigningPackageAsync(packagePath, normalizedKeyId, expectedKeyVersion, cancellationToken);
    ...
    private string RequireDirectory()
    {
        ...
        {
            throw new InvalidOperationException("A package directory must be configured for encrypted-file client signing mode.");
        }
    ...
    private void EnsureWatcher()
    {
        ...
        lock (_watcherSync)
        {
            ...
            _watcher = new FileSystemWatcher(directory, "*.client.acmppkg.json")
            {
    ...
    {
        if (TryExtractKeyId(eventArgs.Name, ".client.acmppkg.json", out var keyId))
        {
            _staleKeys[keyId] = 0;
        }
    }
    ...
    private void OnPackageFileRenamed(object? sender, RenamedEventArgs eventArgs)
    {
        ...
        if (TryExtractKeyId(eventArgs.OldName, ".client.acmppkg.json", out var oldKeyId))
        {
    ...
    private sealed record CacheEntry(
        ClientSigningCredentialPackage Package,
